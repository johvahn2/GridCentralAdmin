...
@Component({
  selector: 'app-ad-explore',
  templateUrl: './ad-explore.component.html',
  styleUrls: ['./ad-explore.component.css'],
  providers:[AdService]
})
export class AdExploreComponent {
  ...
  Add_ad(){
    ...
    if(this.newAd.Location == this.Locations[0]){
      var query = "?Type=Explore&Section=Header";
    }else if(this.newAd.Location == this.Locations[1]){
      var query = "?Type=Explore&Section=SubHeader";
  ...
  Initilize(){
     this.Locations = ["explore-header","explore-subheader"]
